...
class BrokerOptionsTestCase(unittest.TestCase, OptionTestUtils):
    ...
    def _check_server_endpoints(self, option_name, default_port):
        ...
        self.config.parseOptions(
            self.cmd_line + [f"--{option_name}", f"--{option_name}", "tcp:1"]
        )
        ...
        self.config.parseOptions(self.cmd_line + [f"--{option_name}", "unix:/test"])
        self.assertEqual(len(self.config[option_name]), 1)
    ...
    def _check_whitelist(self, option_name):
        ...
        self.config.parseOptions(self.cmd_line)
        check_wl_contents(self.config[config_name], [ip_network(NET_DEFAULT)])

        # If a single argument is supplied, it replaces the default.
        self.config.parseOptions(self.cmd_line + [f"--{option_name}", NET1])
        check_wl_contents(self.config[config_name], [ip_network(NET1)])

        # If two arguments is supplied, use both
        self.config.parseOptions(self.cmd_line + [f"--{option_name}", NET1, NET2])
        check_wl_contents(
            self.config[config_name], [ip_network(NET1), ip_network(NET2)]
        )

        # If --receive-whitelist is supplied twice, use only the second.
        self.config.parseOptions(
            self.cmd_line + [f"--{option_name}", NET1, f"--{option_name}", NET2]
        )
    ...
    def test_eventdb(self):
        ...
        self.assertTrue(
            os.path.isdir(self.config.parseOptions(self.cmd_line)["eventdb"])
        )

        # Specifying our own directory should also work.
        dirname = "/example"
        self.assertEqual(
            self.config.parseOptions(["--eventdb", dirname])["eventdb"], dirname
        )
    ...
    def test_broadcast_test_interval(self):
        ...
        self.config.parseOptions(self.cmd_line)
        self.assertEqual(self.config["broadcast_test_interval"], BCAST_TEST_INTERVAL)

        # And can be over-ridden.
        self.config.parseOptions(self.cmd_line + ["--broadcast-test-interval", 0])
    ...
    def test_subscribe(self):
        ...
        # subcribers.
        self.config.parseOptions(
            self.cmd_line + ["--subscribe", f"tcp:test:{DEFAULT_SUBSCRIBE_PORT}"]
        )
        ...
        self.config.parseOptions(
            self.cmd_line + ["--subscribe", f"test:{DEFAULT_SUBSCRIBE_PORT}"]
        )
        self.assertEqual(len(self.config["subscribe"]), 1)
        self.assertEqual(self.config["subscribe"][0]._port, DEFAULT_SUBSCRIBE_PORT)
        self.assertEqual(self.config["subscribe"][0]._host, "test")
        ...
        self.assertEqual(self.config["subscribe"][0]._port, DEFAULT_SUBSCRIBE_PORT)
        self.assertEqual(self.config["subscribe"][0]._host, "test")
        self.assertEqual(self.config["subscribe"][1]._port, DEFAULT_SUBSCRIBE_PORT + 1)
    ...
    def test_filter(self):
        ...
        self.config.parseOptions(self.cmd_line + ["--filter", "foo", "--filter", "bar"])
    ...
    def test_cmd(self):
        ...
        self.config.parseOptions(self.cmd_line + ["--cmd", "foo", "--cmd", "bar"])
    ...
    def test_save_event_plugin_takes_args(self):
        ...
        self.config.parseOptions(
            self.cmd_line + ["--save-event", "--save-event-directory=/tmp"]
        )
        self.assertEqual(self.config["handlers"][0].name, "save-event")
...
class BrokerServiceTestCase(unittest.TestCase):
    ...
    def test_has_services(self):
        ...
        # A single receiver.
        service = self._make_service(["--local-ivo", "ivo://comet/test", "--receive"])
        self.assertEqual(len(service.services), 1)

        # A single broadcaster.
        service = self._make_service(["--local-ivo", "ivo://comet/test", "--broadcast"])
        self.assertEqual(len(service.services), 1)

        # A single subscriber.
        service = self._make_service(
            ["--local-ivo", "ivo://comet/test", "--subscribe", "tcp:test:12345"]
        )
        self.assertEqual(len(service.services), 1)
