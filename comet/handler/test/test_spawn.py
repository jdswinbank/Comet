...
class SpawnCommandProtocolTestCase(unittest.TestCase):
    ...
    @skipIf(
        sys.platform == "win32" and not win32api,
    ...
    def _check_logged_value(self, script_name, sentinel, should_fail=False):
        ...
        def _check_log(_):
            self.assertTrue(sentinel in " ".join(msg[0] for msg in observer.messages))
    ...
    def test_write_data(self):
        ...
        spawn = SpawnCommand(
            SHELL, util.sibpath(__file__, "test_spawn_output.sh"), output_file.name
        )
        d = spawn(dummy_event)
