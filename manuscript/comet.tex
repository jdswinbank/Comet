...
the subscriber, but also enables subscribers to focus simple, well-targeted
...
$1\times10^7$ events per night: assuming those events are evenly spaced over a
12 hour period, this is equivalent to over 230 events per second. This is the
...
broker seems plausible, although it is worth emphasizing that this calculation
takes no account of follow-up traffic generated in response to events and that
the events used in this benchmark did not carry a scientific payload, and
hence are likely significantly smaller than those which might be produced by
LSST.
