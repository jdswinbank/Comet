...
\begin{enumerate}
  ...
  \item{The document text is parsed into an internal data structure;}
...
\begin{minipage}{252pt} % Minipage equal to width of column contains footnotes
\begin{center}
\caption{Timing measurements for each stage of Comet's processing of 16425
VOEvent documents collected as described in \S\ref{sec:perf:individual}.}
\label{tab:perf:individual}
\begin{tabular}{lcc}
\toprule
Processing stage & \multicolumn{1}{c}{Total (s)} &\multicolumn{1}{c}{Per event (s)} \\
\midrule
Event parsing & 1.625465 & 0.000099 \\
SHA-1 calculation & 0.152024 & 0.000009 \\
\multicolumn{3}{l}{Event database check:\footnote{Also includes SHA-1 calculation.}\footnote{Measured using synthetic data.}} \\
...
All 16425 test VOEvent documents were read from disk and stored as textual
data in memory. Each in turn was parsed into Comet's internal VOEvent
representation\footnote{Comet represents XML documents using a custom-built
wrapper around the \texttt{Element} class provided by lxml
...
The experiment was repeated ten times. The lowest time measured (corresponding
to the fastest performance) was recorded. This is the best representation of
the intrinsic time taken to parse an event: other, slower, measurements are
due to contention for resources from other processes on the test machine.

The time taken to process all of the events, together with the mean time per
event, are recorded in Tab. \ref{tab:perf:individual}.

\subsubsection{SHA-1 calculation}

All 16425 test VOEvent documents were read from disk, parsed, and stored in
memory using Comet's internal representation. The total time taken to
calculate the 40 byte hexadecimal SHA-1 hash for each event in turn was
measured.

The experiment was repeated ten times; as in the previous section, we regard
the fastest repetition as the best representation of the true processing time.
The total time for all events, and the mean time per event, are recorded in
Tab. \ref{tab:perf:individual}

\subsubsection{Comparision to event database}
...
processing events 1540000--1549999, at an average of 0.000491\,s per event;
the highest when processing events 8540000--8549999, at an average of
0.000502\,s per event. There was no systematic increase in processing time
with event database size. Testing with a significantly larger database was
impossible due to the available memory.

\subsubsection{Schema validation}


\subsubsection{XPath filtering}
...
%
% TODO: Rework; note that tmpfs is now described in an earlier section.
%
