...
\multicolumn{3}{l}{Event database check:\footnote{Also includes SHA-1 calculation.}} \\
...
to the fastest performance) and the corresponding mean parse time per document
is recorded in Tab.~\ref{tab:perf:individual}. This is the best representation
of the intrinsic time taken to parse an event: other, slower, measurements are
...
We record this fastest total time for all events, together with the
corresponding mean hash calculation time per event, in
...
As in \S\S\ref{sec:perf:individual:parse} and \ref{sec:perf:individual:hash},
the experiment was repeated ten times and the fastest result recorded in
Tab.~\ref{tab:perf:individual}.
...
As described in previous sections, each experiment was repeated ten times and
the fastest result for each expression is recorded in
Tab.~\ref{tab:perf:individual}.
